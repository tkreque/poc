...
resource "aws_security_group_rule" "RDS_ingress" {
  for_each          = { for index, rds in var.configs.env_vars.rds : rds.name => rds }
  security_group_id = aws_security_group.rds[each.key].id
  description       = "Allow communication with the RDS."
  type              = "ingress"
...
resource "aws_security_group_rule" "RDS_ingress_from_internal" {
  for_each          = { for index, rds in var.configs.env_vars.rds : rds.name => rds }
  security_group_id = aws_security_group.rds[each.key].id
  description       = "Allow internal communication with the RDS."
  type              = "ingress"
  from_port         = each.value.access.port
  to_port           = each.value.access.port
  protocol          = "tcp"
  cidr_blocks       = var.configs.env_vars.vpc.subnets.private
}